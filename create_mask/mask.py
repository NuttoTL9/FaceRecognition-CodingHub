...
def overlay_transparent(background, overlay, x, y):
    """ ฟังก์ชันวางภาพโปร่งใส overlay ลงบน background """
    h, w = overlay.shape[:2]
    
    if x + w > background.shape[1] or y + h > background.shape[0]:
        return background  # ไม่วางเกินขอบภาพ
...
def create_mask_single(image, mask_path):
    """รับ image ที่เปิดมาแล้ว และวาง mask ลงไป"""
    mask = cv2.imread(mask_path, cv2.IMREAD_UNCHANGED)
    if mask is None:
        print(f"Cannot read mask: {mask_path}")
        return image

    # ปรับขนาดหน้ากากให้พอดีกับใบหน้า สมมุติครอบบนกลางภาพ
    scale_factor = 0.5
    new_w = int(image.shape[1] * scale_factor)
    new_h = int(mask.shape[0] * (new_w / mask.shape[1]))
    mask = cv2.resize(mask, (new_w, new_h))

    x = (image.shape[1] - new_w) // 2
    y = int(image.shape[0] * 0.5)

    image = overlay_transparent(image, mask, x, y)
    return image
